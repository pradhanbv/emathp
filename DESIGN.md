...
(in-process → DuckDB → shared ClickHouse → Spark serverless), and capacity math derived
...
- **The executor is now two executors**, selected by `--join-engine`: the cgo-free Go hash join
  (default) and [ADR-007](#adr-007--join-strategy-a-four-tier-escalation-ladder) tier 1's embedded
  DuckDB, one instance per query with `threads=1`. Both satisfy one `JoinEngine` contract, and
  `TestEnginesAgreeOnFourWayJoin` requires identical output from each. **N-way joins execute** —
  four tables and three links, through either engine. What is still hand-rolled is everything
  *around* the merge, which is where the remaining limits sit.
...
**Tiers 0–1 are in-process; tiers 2–3 are separate, standing compute** — a warm ClickHouse
cluster and a serverless Spark account, both fed by S3 staging and driven by the async job
runner, which is why they appear as their own data-plane row rather than inside the gateway.
...
|---|---|---|---|
| **1** | Native `ETag` / `If-Modified-Since` conditional request | 1 call, 304 on hit — the probe *is* the fetch, so it never doubles | ✅ |
| **2** | Change/event feed or cursor API (e.g. Zendesk incremental exports), maintained as a replica rather than probed per query | 0 calls per query once synced; replication runs on its own schedule, not charged to any single request | ✅, only if the feed emits explicit delete events |
| **3** | `updated_after` filter + 1-row sorted fetch as watermark | 1 call if unchanged; 2+ calls if changed (the probe, then a full live fetch) | ❌ — pair with periodic full-refresh floor |
| **4** | None — TTL only | 0 calls | ❌ |

- **Quota accounting.** A rung-1 or rung-3 probe spends a rate-limit token, charged to the same
  bucket as a data fetch and visible in `rate_limit_status`. Freshness that silently consumes
  quota is a quota leak with good PR, not freshness control. Rung 2 doesn't spend a token per
  query at all — its cost is a background replication job, on its own schedule, not charged to
  any single request.
...
- **Rung 2 needs its own component, not another branch of the per-query cache.** Rungs 1, 3, and
  4 all work the same way: check how old the cached entry is, maybe send one probe, then either
  serve the cached rows or fetch live. A change feed doesn't fit that shape. Its natural use is
  keeping one full local copy of a table continuously up to date, independent of any specific
  query, and then answering any filtered query from that copy with no extra calls. That's a
  background replicator, not a probe — a different kind of component from what the other three
  rungs share.
- **A rung-2 replica can't be shared across principals.** [ADR-002](#adr-002--entitlement-enforcement-the-briefs-hardest-requirement)'s
  L3 layer sends every connector call under the calling principal's own delegated token, so the
  source enforces that principal's own sharing rules — Salesforce record ownership, Zendesk
  group restrictions. A replica built with one principal's token only reflects what that
  principal can see. Using it to answer a different principal's query would either leak rows the
  second principal's own access wouldn't return, or hide rows they should see. So each principal
  needs their own replica. That still saves calls across that principal's own different queries —
  it just never becomes one shared copy for everyone.
- **The cache never holds a join result, only single tables.** `freshness.Source` wraps each
  connector separately. Every fetch it sees — a plain scan, a join's first side, or one
  semi-join probe chunk — is cached on its own, keyed by that one table's
  `(principal, columns, filters)`. The join itself happens afterward, once every side has
  already come back, and that merged result is never cached. This has a side effect: each side
  of a join ages on its own clock, so one side's cached rows might be fresh while another's are
  close to the staleness limit. Each side is still within its own budget individually. But the
  two sides together aren't guaranteed to reflect the same moment in time.
...
| **1. DuckDB** (in-process, any join) | • Fits: working set within the gateway pod's shared memory<br>• Doesn't: exceeds it → `RESULT_TOO_LARGE`, or suggest `Prefer: respond-async` if tier 2 fits | • **One instance per query**, not one shared per pod — `memory_limit` binds a buffer manager, so this is what lets the pod budget **8 concurrent joins × 256 MB** as eight real ceilings rather than one pool they divide ([§6.3](#63-gateway-pod-size--worked-backwards-not-asserted))<br>• **`threads` capped low** — DuckDB parallelises *within* a query, and eight concurrent joins each claiming a core oversubscribes the pod; the vectorised execution model still wins single-threaded<br>• **Bulk-loaded through DuckDB's appender, never row-at-a-time** — the appender writes into DuckDB's internal data chunks and flushes per vector, so the cgo boundary is crossed per chunk rather than per row or per `INSERT`. (Arrow would be the alternative, but ingesting through it needs an `array.RecordReader`, i.e. taking on `apache/arrow-go` to describe data that is entirely `VARCHAR`)<br>• Explicit `memory_limit`, per-tenant-encrypted ephemeral temp dir, reset every query<br>• Semi-join rewrite minimizes what's loaded; nothing survives the request |
...
| **Tier 3 Spark, not a bigger ClickHouse node** | ClickHouse's `GLOBAL JOIN` degenerates to "one side must fit everywhere"; Spark's shuffle-based join scales by adding executors |

- **Tiers 2–3 need a real async job runner**, not the prototype's in-memory map. The
  `Prefer: respond-async` contract (202 + poll URL) exists; what a warm pool adds is state that
  survives a pod restart. It stages already-filtered rows to the tenant's S3 prefix, admits against
  per-tenant cluster limits (reusing [ADR-006](#adr-006--rate-limiting-and-multi-tenant-fairness)'s
  semaphores and fair queues), dispatches, tracks status, **deletes staged objects on completion or
  failure** — a leaked stage is customer data left in durable storage — and cancels in flight, since
  off-boarding drains async queues
...
- **`JoinEngine` stops at tier 1 — by necessity, not omission.** The interface takes
  ...
  this interface would mean first doing the thing tier 2 avoids. Three more properties invert at
  the same boundary: the call is synchronous where tiers 2–3 are `202`-and-poll, the result comes
  back in memory where a tier-3 result can exceed the pod, and `JoinInput` carries an alias but no
  tenant — which staging needs for the per-tenant prefix and KEK
  ([ADR-010](#adr-010--keys-crypto-shredding-and-the-audit-conflict)). Widening one interface to
  span both would force tier 1 to pay job-submission overhead for a ~100 ms merge, or tier 2 to
  block a request thread for minutes.
- **So tiers 2–3 get a second interface, not a wider one** — `Submit(tenant, staged, links) →
  JobID` plus `Poll(JobID)`, with the tier router choosing between them. Two things carry across
  unchanged, which is what keeps the split cheap: **the join graph** (`Links` holds *indices* into
  sides, not node pointers, so the same object describes the work whether DuckDB or ClickHouse
  runs it), and **the fetch cascade** — tiers 2–3 still fetch every side filtered, still benefit
  from the semi-join reduction, and only then stage instead of merging. The one signature change
  that split needs is a sink: `fetchSides(..., sink RowSink)`, where the sink is an in-memory
  collector for tier 1 and a Parquet writer for tiers 2–3, since at tier-3 scale a single side may
  not fit in memory either.
- **"Spill to materialization" — which reading?** *Materialized intermediate* — yes, tier 1.
  *Disk spill mid-join* — no for tiers 0–2: 0–1 trade a fast failure for a slow one, tier 2 needs
  its ceiling estimable at plan time. Only tier 3 spills, and there it is the vendor engine's
  problem.
...
- **N-way joins are built.** Four tables, three links, through either engine — `TestFourTableJoinEndToEnd`
  proves it over HTTP and `TestDuckDBIsNWayCapable` proves the engine was never the constraint.
  The 2-table cap turned out to span three layers, not the one an earlier draft claimed: a parse
  restriction, `plan` collapsing join sides to left/right instead of carrying N aliases, and
  `JoinEngine` being 2-way by signature. Fixing all three is what made it work; deleting the parse
  check alone would not have. `Join` now carries `Sides` plus N-1 `Links`, and `ProjectCol.Side` is
  the **alias** rather than `"L"`/`"R"` — the alias was always available, and collapsing it is what
  made the merge non-composable.
- **What N-way still lacks is *ordering*, not capability.** The cascade is left-deep in FROM order,
  and each join must reference an earlier table (`TestJoinRejectsForwardReference`) so a probe's
  IN-list is always built from rows already fetched. Choosing a better order needs cost-based join
  ordering — the planner [ADR-001](#adr-001--planner-runtime-proposed) defers — and an N-way
  working-set estimate good enough to route between tiers. That estimate also makes routing
  *harder*: cardinality error compounds as tables chain.
...
- **Materialization's shred surface, per tier:** tiers 0–2 hold **nothing on local disk** — tier 1
  is in-process, tier 2 runs strictly in memory ([ADR-007](#adr-007--join-strategy-a-four-tier-escalation-ladder)).
  What tiers 2–3 leave is the staged input on **S3 under a per-tenant prefix and the tenant's KEK**,
  tier 3's output written back the same way — one envelope, both directions, so disabling that KEK
  makes both unreadable. Tier 3's shuffle spill is the ladder's only local-disk surface, and it dies
  with its ephemeral executors.
...
| Tier-1 `RESULT_TOO_LARGE` rate forces [ADR-007](#adr-007--join-strategy-a-four-tier-escalation-ladder) tier 2 before GA | Med | Med | Not built by default; pull into M5/M6 if triggered — a warm cluster plus S3 staging and the async orchestrator, so it is a real standing cost, which is why the trigger gates it | BE(1) | Fires on > 5% of cross-app joins in M4 |
...
fee**. **Tiers 2–3 are a standing line** once escalation volume justifies them — a warm ClickHouse cluster plus S3 staging — sized by escalation rate rather than by QPS, and deferred until the risk register's trigger fires.
