...
| **007** Joins | *"federated on the fly vs. short-lived materialization"*; *"spill to materialization when necessary"* | Cross-app joins are the product's reason to exist but cannot be pushed to any single source. | • Container-per-join DuckDB<br>• Naive dual full fetch<br>• Always-on shared ClickHouse cluster<br>• Spark-only from day one, skipping ClickHouse | **Partial** - semi-join yes; DuckDB, ClickHouse, and Spark tiers designed, none built |
