...
| **Spark-only from day one** (no tier 2 at all) | Spark can do everything ClickHouse can; tier 2 adds no *capability*, only cost efficiency. One async engine instead of two is less to build, operate and reason about. | Inside its band a warm pool of dedicated nodes beats per-job serverless overhead, and tier 2's ceiling is a number the plan-time estimate can compare against. | **The rejection this design weakened most.** Tier 2 is now a cost optimization inside a utilization window, not a capability rung - and serialization pins node count to concurrent jobs, so above ~1/s escalations its economics fail and tier 3 is the answer anyway. Still rejected, by a narrower margin than when written. |
